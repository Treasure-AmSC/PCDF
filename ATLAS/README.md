...
The final wizard step can also generate a CPU-only SLURM wrapper targeted at NERSC Perlmutter. Download both the Python script and `submit-pcdf-ntuple.slurm`, copy them to Perlmutter, create a manifest with one input DAOD path per line, edit the NERSC project account plus manifest/output paths, and submit with `sbatch submit-pcdf-ntuple.slurm` from a Perlmutter login node. The tar bundle marks the generated Python and SLURM scripts executable. The wrapper requests a wider CPU allocation and uses GNU parallel inside the job to distribute input files across nodes without creating a large SLURM job array. Conversions per node controls parallelism; CPUs per conversion are derived from Perlmutter CPU nodes (128 CPU cores per node), rather than entered independently.
